...
# Import vis.js network component
try:
    from neovis_component_fixed import create_visjs_network_component
    VISJS_AVAILABLE = True
except ImportError:
    VISJS_AVAILABLE = False
    create_visjs_network_component = None
    ...
    **Try these examples:** aspirin, insulin, morphine, ibuprofen, metformin
    ...
    if search_term:
        ...
        if drug_details:
            ...
            # Get network data for vis.js visualization (works for drug-centered view)
            visjs_network_data = None
            if center_node == selected_drug:
                # Get network data for the selected drug
                cache_key_visjs = f"drug_network_visjs_{selected_drug}"
                if cache_key_visjs in st.session_state:
                    visjs_network_data = st.session_state[cache_key_visjs]
                else:
                    visjs_network_data = app.get_drug_network(selected_drug)
                    if visjs_network_data:
                        st.session_state[cache_key_visjs] = visjs_network_data
            ...
            if targets:
                ...
                # Display the interactive network with tabs for both visualizations
                
                # Only show tabs if we have vis.js available and network data for drug-centered view
                if VISJS_AVAILABLE and visjs_network_data and center_node == selected_drug:
                    tab1, tab2 = st.tabs(["📊 Plotly Network", "🌐 Interactive vis.js Network"])
                    
                    with tab1:
                        st.plotly_chart(fig, use_container_width=True)
                    
                    with tab2:
                        st.info("🖱️ **Interactive Features:** Drag nodes • Hover for details • Click to focus • Right-click for options")
                        try:
                            visjs_html = create_visjs_network_component(selected_drug, visjs_network_data, height=800)
                            # Use components.v1.html for better compatibility across Streamlit versions
                            import streamlit.components.v1 as components
                            components.html(visjs_html, height=820, scrolling=False)
                        except Exception as e:
                            st.error(f"Error rendering vis.js network: {e}")
                            st.info("Falling back to Plotly visualization")
                            st.plotly_chart(fig, use_container_width=True)
                else:
                    # Default: Show only Plotly
                    st.plotly_chart(fig, use_container_width=True)
                    
                    # Show info if vis.js is not available
                    if not VISJS_AVAILABLE:
                        st.caption("💡 Tip: Install neovis_component_fixed.py to enable interactive vis.js network visualization")
                    elif center_node != selected_drug:
                        st.caption("💡 Tip: vis.js visualization is available for drug-centered view. Reset to drug center to see it.")
        ...
        if app.driver:
            ...
            with app.driver.session(database=app.database) as session:
                ...
                if mechanism_stats:
                    ...
                    st.info("📊 **No classification statistics available yet.** Start classifying drug-target relationships to see statistics here.")
            ...
            st.warning("🔌 **Please connect to Neo4j first** to view classification statistics.")
